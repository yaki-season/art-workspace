...
const topologyRegistry = JSON.parse(
  await readFile(path.join(pipelineDir, contract.topologyRegistry), "utf8"),
);
try {
  await validateSchema(
    "screen-topology-registry",
    topologyRegistry,
...
const topologyByMaster = new Map(
  topologyRegistry.entries.map((entry) => [entry.sourceMasterId, entry]),
);
if (topologyByMaster.size !== topologyRegistry.entries.length) {
  errors.push("pipeline topology registry: sourceMasterId가 중복됩니다");
}
...
for (const reportPath of reports) {
  ...
  if (!report.spatialInference?.file || !report.spatialInference?.sha256) {
    errors.push(`${relativeReport}: spatial inference evidence is missing`);
  } else {
    const inferencePath = path.resolve(
      reportDir,
      report.spatialInference.file,
    );
    try {
      const inferenceBuffer = await readFile(inferencePath);
      const inferenceSha256 = createHash("sha256")
        .update(inferenceBuffer)
        .digest("hex");
      if (inferenceSha256 !== report.spatialInference.sha256) {
        errors.push(`${relativeReport}: spatial inference SHA-256 mismatch`);
      }
      const inference = JSON.parse(inferenceBuffer.toString("utf8"));
      const inferenceSchema = await schemaValidator("spatial-inference");
      if (!inferenceSchema(inference)) {
        errors.push(
          `${relativeReport}: spatial inference schema 오류 (${schemaErrors(inferenceSchema).join("; ")})`,
        );
      }
      if (inference.approvedForGeneration !== true) {
        errors.push(`${relativeReport}: spatial inference is not approved`);
      }
      if (inference.sourceMasterId !== report.sourceMasterId) {
        errors.push(`${relativeReport}: spatial inference sourceMasterId mismatch`);
      }
      if (
        inference.profile !== report.profile
        || inference.screenUnit !== report.screenUnit
      ) {
        errors.push(`${relativeReport}: spatial inference profile 또는 screenUnit mismatch`);
      }
      if (
        !Array.isArray(inference.screen?.screenIds) ||
        inference.screen.screenIds.length === 0 ||
        !inference.screen?.gameState ||
        !inference.screen?.playerAction ||
        !inference.screen?.semanticOwner ||
        !inference.screen?.assetMeaning
      ) {
        errors.push(`${relativeReport}: screen and asset meaning are incomplete`);
      }
      if (
        !Array.isArray(inference.settingSources) ||
        inference.settingSources.length === 0 ||
        inference.settingSources.some(
          (source) => !source.file || !source.sha256 || !source.evidence,
        )
      ) {
        errors.push(`${relativeReport}: setting source evidence is incomplete`);
      } else {
        for (const source of inference.settingSources) {
          try {
            await verifyWorkspaceEvidence(
              source,
              `${relativeReport}: setting source ${source.file}`,
            );
          } catch (error) {
            errors.push(error.message);
          }
      ...
      if (
        !inference.camera?.position ||
        !inference.camera?.viewDirection ||
        !inference.camera?.framing
      ) {
        errors.push(`${relativeReport}: camera inference is incomplete`);
      }
      for (const field of [
        "spatialOrder",
        "occludedContinuation",
        "invariants",
        "forbiddenSubstitutions",
      ]) {
        if (!Array.isArray(inference[field]) || inference[field].length === 0) {
          errors.push(`${relativeReport}: ${field} inference is incomplete`);
        }
      }
      if (
        !Array.isArray(inference.unresolvedQuestions) ||
        inference.unresolvedQuestions.length > 0
      ) {
        errors.push(`${relativeReport}: unresolved spatial questions remain`);
      }

      const topologyEntry = topologyByMaster.get(report.sourceMasterId);
      if (!topologyEntry) {
        errors.push(`${relativeReport}: sourceMaster topology가 registry에 없습니다`);
      } else if (topologyEntry.approvalStatus !== "approved-by-user") {
        errors.push(
          `${relativeReport}: sourceMaster topology가 승인되지 않았습니다 (${topologyEntry.approvalStatus})`,
        );
      } else if (!topologyEntry.screenUnits.includes(report.screenUnit)) {
        errors.push(`${relativeReport}: screenUnit이 sourceMaster topology 범위에 없습니다`);
      } else if (
        inference.topologyRegistryEntry?.sourceMasterId
          !== topologyEntry.sourceMasterId
        || inference.topologyRegistryEntry?.approvalStatus
          !== topologyEntry.approvalStatus
        || inference.topologyRegistryEntry?.approvalDate
          !== topologyEntry.approval?.date
      ) {
        errors.push(`${relativeReport}: topology registry 승인 증거가 현재 registry와 다릅니다`);
      } else {
        const topologyRule = topologyEntry.topology;
        if (
          JSON.stringify(inference.spatialOrder) !==
          JSON.stringify(topologyRule.spatialOrder)
        ) {
          errors.push(`${relativeReport}: spatial order violates screen topology`);
        }
        if (
          inference.topology?.mainEntranceRelation !==
          topologyRule.relations.mainEntranceRelation
        ) {
          errors.push(`${relativeReport}: main entrance topology is wrong`);
        }
        if (
          inference.topology?.entranceComposition !==
          topologyRule.relations.entranceComposition
        ) {
          errors.push(`${relativeReport}: entrance composition is wrong`);
        }
        if (
          inference.topology?.sideWallsRelation !==
          topologyRule.relations.sideWallsRelation
        ) {
          errors.push(`${relativeReport}: side wall topology is wrong`);
        }
        const missingForbidden = topologyRule.forbiddenSubstitutions.filter(
          (item) => !inference.forbiddenSubstitutions?.includes(item),
        );
        if (missingForbidden.length > 0) {
          errors.push(
            `${relativeReport}: forbidden substitutions missing (${missingForbidden.join(", ")})`,
          );
        }
      }
    } catch (error) {
      errors.push(
        `${relativeReport}: spatial inference file invalid (${error.message})`,
      );
    }
  }

...
body.name = "negi-body"; body.scale.set(0.30, 0.30, 0.30); body.position.z = -0.035; body.rotation.z = Math.PI / 2; model.add(body);
const decal = new Mesh(new PlaneGeometry(0.255, 0.192), new MeshBasicMaterial({ color: 0xffffff, transparent: true, side: DoubleSide }));
decal.name = "pixel-material-plane"; decal.position.z = 0.078;
// R2 displayed upright only before the horizontal skewer-parent rotation.
// -26° aligns the source pixels to local X, so the real -90° assembly pose
// renders the long green-onion silhouette vertically across a horizontal skewer.
decal.rotation.z = -26 * Math.PI / 180; model.add(decal);
...
await writeFile(path.join(assetDir,"mdl-ingredient-negi-r3.glb"),Buffer.from(glb));
const inspection={id:"MDL-INGREDIENT-NEGI",format:"glb + external pixel albedo",coordinateSystem:"meters, Y-up, +Z front",origin:"skewerSocket",skewerAxis:"local +Y; crosses the negi tube laterally",bodyLongAxis:"local X; perpendicular to skewer axis",assemblyPose:"MDL-SKEWER-BASE parent rotation Z=-90° makes the local X long axis screen-vertical",requiredNodes:["skewerSocket"],materialBinding:{targetNode:"pixel-material-plane",albedo:"tex-ingredient-negi-albedo-r3.png",sampling:"nearest",colorSpace:"sRGB"},cutFaceUV:"pixel-material-plane normalized UV [0,1]×[0,1]",boundsMeters:{width:0.255,height:0.22,depth:0.156},triangles:triangles(model),triangleBudget:800,status:"within-budget"};
await writeFile(path.join(assetDir,"mdl-ingredient-negi-r3.inspection.json"),`${JSON.stringify(inspection,null,2)}\n`);console.log(JSON.stringify(inspection));

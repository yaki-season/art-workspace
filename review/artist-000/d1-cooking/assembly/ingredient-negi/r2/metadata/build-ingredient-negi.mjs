...
// The source segment is oblique. Rotate its pixel plane by the measured 64°
// correction so the visible long silhouette is exactly screen-vertical, while
// the depth body remains on local X and the skewer crosses on local +Y.
...
await writeFile(path.join(assetDir,"mdl-ingredient-negi-r2.glb"),Buffer.from(glb));
const inspection={id:"MDL-INGREDIENT-NEGI",format:"glb + external pixel albedo",coordinateSystem:"meters, Y-up, +Z front",origin:"skewerSocket",skewerAxis:"local +Y; crosses the negi tube laterally",requiredNodes:["skewerSocket"],materialBinding:{targetNode:"pixel-material-plane",albedo:"tex-ingredient-negi-albedo-r2.png",sampling:"nearest",colorSpace:"sRGB"},cutFaceUV:"pixel-material-plane normalized UV [0,1]×[0,1]",boundsMeters:{width:0.255,height:0.22,depth:0.156},triangles:triangles(model),triangleBudget:800,status:"within-budget"};
await writeFile(path.join(assetDir,"mdl-ingredient-negi-r2.inspection.json"),`${JSON.stringify(inspection,null,2)}\n`);console.log(JSON.stringify(inspection));

...
const sourceRoot = path.resolve(here, "../../../../negima-cooking-second-face/r3");
...
async function measureR3Signal(page, { width, height }) {
  return page.evaluate(async ({ targetWidth, targetHeight }) => {
    ...
    const [r1, r3] = await Promise.all([
      ...
      load(`/art-workspace/review/artist-000/d1-cooking/grill/recomposition/cooking-second-face-station-consumption/r3/review-mdl-negima-grill-cooking-second-face-station-${suffix}-r3.png`),
    ]);
    ...
    context.drawImage(r3, 0, 0, targetWidth, targetHeight);
    const r3Pixels = context.getImageData(0, 0, targetWidth, targetHeight).data;
    ...
    for (let y = region.y; y < region.y + region.height; y += 1) for (let x = region.x; x < region.x + region.width; x += 1) {
      ...
      const redDelta = r3Pixels[offset] - r1Pixels[offset];
      const greenDelta = r3Pixels[offset + 1] - r1Pixels[offset + 1];
      const blueDelta = r3Pixels[offset + 2] - r1Pixels[offset + 2];
      const delta = Math.abs(redDelta) + Math.abs(greenDelta) + Math.abs(blueDelta);
      if (delta > 28) signalPixels += 1;
      if (delta > 38 && (r3Pixels[offset] > r3Pixels[offset + 2] + 18 || r3Pixels[offset + 1] > r3Pixels[offset + 2] + 14)) amberOrCaramelSignalPixels += 1;
...
try {
  ...
  for (const item of cases) {
    ...
    page.on("response", (response) => { if (response.status() >= 400) console.error(`browser-response:${response.status()}:${response.url()}`); });
    page.on("pageerror", (error) => console.error(`browser-pageerror:${error.message}`));
    page.on("console", (message) => { if (message.type() === "error") console.error(`browser-console:${message.text()}`); });
    await page.goto(`http://127.0.0.1:8012/art-workspace/review/artist-000/d1-cooking/grill/recomposition/cooking-second-face-station-consumption/r3/review-mdl-negima-grill-cooking-second-face-station-r3.html?viewport=${item.name}`, { waitUntil: "networkidle" });
    ...
    const state = await page.evaluate(() => window.__cookingSecondR3StationConsumption.state());
    ...
    if (!sameBox(box, item.expected)) throw new Error(`${item.name}: R3 shader must not change raw R3 footprint: ${JSON.stringify({ box, expected: item.expected })}`);
    ...
    if (state.shader.face1HeatProgress !== 0.52 || signal.chickenClusters !== 3 || signal.greenOnionBands !== 2 || JSON.stringify(signal.coarseUvCells) !== JSON.stringify([16, 16]) || !signal.signalOnlyOnVisibleFace || state.shader.materialBindingCount !== 5 || !state.shader.reverseDecalPreserved || !state.shader.nearestAlbedosReused || state.shader.newRaster || state.shader.newGlb || state.shader.newTexture || state.shader.newAtlas || state.shader.newBake) throw new Error(`${item.name}: R3 shader or source policy mismatch`);
    const output = path.join(root, `review-mdl-negima-grill-cooking-second-face-station-${item.name}-r3.png`);
    await page.screenshot({ path: output, clip: { x: 0, y: 0, width: item.width, height: item.height } });
    const signalCheck = await measureR3Signal(page, item);
    if (signalCheck.signalPixels < item.minSignalPixels || signalCheck.amberOrCaramelSignalPixels < item.minSignalPixels / 3 || signalCheck.maxChannelDelta < 36) throw new Error(`${item.name}: R3 face signal is not strong enough at station scale: ${JSON.stringify(signalCheck)}`);
  ...
  await comparison.goto("http://127.0.0.1:8012/art-workspace/review/artist-000/d1-cooking/grill/recomposition/cooking-second-face-station-consumption/r3/review-mdl-negima-grill-cooking-face-compare-fhd-r3.html", { waitUntil: "networkidle" });
  await comparison.waitForFunction(() => document.body.dataset.ready === "true");
  const parity = await comparison.evaluate(() => window.__cookingFaceR3Comparison);
  if (parity.left !== "approved cooking-first-face station R1" || parity.right !== "cooking-second-face R3") throw new Error(`first/second comparison source mismatch: ${JSON.stringify(parity)}`);
  const comparisonOutput = path.join(root, "review-mdl-negima-grill-cooking-face-compare-fhd-r3.png");
  ...
  const sourceIntegration = {
    ...
    stateFile: { file: "../assets/mdl-negima-grill-cooking-second-face-r3.state.json", sha256: await digest(path.join(sourceRoot, "assets/mdl-negima-grill-cooking-second-face-r3.state.json")) },
  ...
  const report = {
    ...
    status: "pending-user-review",
    reviewBoards: boards,
    comparison: { file: `../${path.basename(comparisonOutput)}`, width: 1920, height: 1080, bytes: (await stat(comparisonOutput)).size, sha256: await digest(comparisonOutput), left: "approved cooking-first-face station R1", right: "cooking-second-face R3", crop: parity.sourceCrop, parity: "same master-derived base, D1 slot0 anchor and shared transform; only the approved R3 reverse-face shader signal differs" },
    r3SignalAgainstR1: { evidence: "R1 and R3 second-face screenshots at the same station placement were compared inside the negima footprint; this proves the visible coarse shader signal without changing geometry or source textures.", views: signalChecks, result: "pass" },
    result: "pass",
    runtimeRegistrationAllowed: false,
  };
  await writeFile(path.join(here, "consumption-validation-report.json"), `${JSON.stringify(report, null, 2)}\n`);
  console.log(`cooking-second R3 station capture complete: ${report.comparison.sha256}`);
} finally {
  await browser.close();
}

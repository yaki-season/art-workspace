...
try {
  ...
  for (const item of cases) {
    ...
    await page.goto(`http://127.0.0.1:8012/art-workspace/review/artist-000/d1-cooking/grill/recomposition/raw-station-consumption/r3/review-mdl-negima-grill-raw-station-r3.html?viewport=${item.name}`, { waitUntil: "networkidle" });
    ...
    if (state.transform.verticalScale !== 1.18 || state.triangleCount !== 476 || state.flipPivot.axis !== "local +Y" || state.flipPivot.firstInput !== "0->PI" || state.flipPivot.secondInput !== "PI->2PI") throw new Error(`${item.name}: transform or model contract mismatch`);
    ...
    if (box.height < safe.height * 0.85) throw new Error(`${item.name}: skewer remains too short: ${JSON.stringify({ box, safe })}`);
    const output = path.join(root, `review-mdl-negima-grill-raw-station-${item.name}-r3.png`);
  ...
  await writeFile(path.join(here, "consumption-validation-report.json"), `${JSON.stringify({ schemaVersion: 1, id: "MDL-NEGIMA-GRILL-RAW", sourceRevision: 1, reviewRevision: 3, status: "pending-user-review", reviewBoards: boards, result: "pass", runtimeRegistrationAllowed: false }, null, 2)}\n`);

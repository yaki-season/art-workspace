...
// Proper chicken stays a light, appetising grey-brown; it must not read as orange or over-charred.
...
// Proper cooking: preserve the approved albedo and derive a light cooked-meat surface with sparse sear.
...
float negimaRoast=negimaFaceMatch*(0.64+0.36*step(0.10,negimaNoise))*(0.82+0.18*negimaCenter);
float negimaChar=negimaFaceMatch*step(0.84,negimaNoise)*smoothstep(0.18,0.72,negimaCenter);
float negimaGloss=negimaFaceMatch*step(0.982,negimaNoise)*smoothstep(0.26,0.64,negimaCenter);

...
const SEAR_TINTS = Object.freeze({
  chicken: new THREE.Color(0x74401f),
  "green-onion": new THREE.Color(0x5d5520),
});
const COOKED_BASE_TINTS = Object.freeze({
  chicken: new THREE.Color(0xb8874f),
  "green-onion": new THREE.Color(0x716c24),
});
...
function createSecondFaceMaterial(source, ingredient, { face0HeatProgress, face1HeatProgress }) {
  ...
  material.onBeforeCompile = (shader) => {
    ...
    shader.uniforms.uNegimaCookedBaseTint = { value: COOKED_BASE_TINTS[ingredient].clone() };
...
uniform vec3 uNegimaCookedBaseTint;
...
float negimaSharedWarmth = negimaHeat * mix(1.02, 0.46, uNegimaIngredientKind);
float negimaSignal = negimaFace1Marks * uNegimaFace1Heat;
diffuseColor.rgb = mix(diffuseColor.rgb, uNegimaCookedBaseTint, negimaSharedWarmth);
diffuseColor.rgb = mix(diffuseColor.rgb, uNegimaSearTint, negimaSignal * (1.0 - uNegimaIngredientKind) * 0.76);
diffuseColor.rgb = mix(diffuseColor.rgb, vec3(0.76, 0.48, 0.20), negimaSignal * (1.0 - uNegimaIngredientKind) * 0.10);
...
diffuseColor.rgb = mix(diffuseColor.rgb, vec3(0.54, 0.37, 0.07), negimaSignal * uNegimaIngredientKind * 0.10);`,
  ...
  material.customProgramCacheKey = () => `negima-cooking-second-face-r3-${ingredient}`;
...
export async function composeMdlNegimaGrillCookingSecondFace({
  ...
  if (face0ElapsedSeconds !== 4 || face1ElapsedSeconds !== 4 || face0HeatProgress !== 0.52 || face1HeatProgress !== 0.52) {
    throw new Error("R3 fixes the approved 4-second per-face / 0.52 heat snapshot; this shader must not imply a later cook stage");
  }
